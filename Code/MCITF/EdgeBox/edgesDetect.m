...
%
% INPUTS
%  I        - [h x w x 3] color input image 3通道彩图
%  model    - structured edge model trained with edgesTrain 已训练的结构边缘模型
%
% OUTPUTS
%  E        - [h x w] edge probability map 边缘概率图
%  O        - [h x w] coarse edge normal orientation (0=left, pi/2=up)方向
...
if( opts.multiscale )
  ...
  I = imPad(I,p,'symmetric'); % impad将原图上下左右各16-pixel宽镜像扩展
  ...
  if(nargout<4), [E,inds] = edgesDetectMex(model,I,chnsReg,chnsSim); % edgesDetectMex获得边缘检测图
