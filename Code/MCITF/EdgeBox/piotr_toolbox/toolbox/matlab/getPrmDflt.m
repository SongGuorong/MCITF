...
% value(指定未包含参数名的默认值). If default value for variable 'name' is 
% 'REQ', and value for'name' is not given, an error is thrown. 
% See below for usage details.
% USAGE (nargout==1)输出参数个数为1
...
if( mod(length(dfs),2) ), error('odd number of default parameters'); end % 参数个数因为偶数
if nargin<=2, checkExtra = 0; end

% get the input parameters as two cell arrays: prmVal and prmField
if iscell(prm) && length(prm)==1, prm=prm{1}; end % prm=dfs
if iscell(prm)
